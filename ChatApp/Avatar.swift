...
private struct AvatarShape: ViewModifier {
  ...
  func body(content: Content) -> some View {
    content
      .foregroundColor(additionalUser ? Color.primary : Color.white)
      .frame(width: diameter,
             height: diameter,
             alignment: .center)
      .background(additionalUser ? Color.black.opacity(0.5) : Color.gray)
      .cornerRadius(radius())
  }
}
...
struct PBAvatar_Previews: PreviewProvider {
    static var previews: some View {
        Group {
            HStack {
                PBAvatar(image: Image(systemName: "person"), size: .xxSmall, status: .online)
                PBAvatar(image: Image(systemName: "person"), size: .xSmall, status: .away)
                PBAvatar(image: Image(systemName: "person"), size: .small, status: .online)
                PBAvatar(image: Image(systemName: "person"), size: .medium, status: .away)
                PBAvatar(image: Image(systemName: "person"), size: .large, status: .online)
                PBAvatar(image: Image(systemName: "person"), size: .xLarge, status: .offline)
            }
            HStack {
          ...
          PBAvatar(image: Image(systemName: "person"), size: .large, shape: .roundedSquare)
        }
    }
}
